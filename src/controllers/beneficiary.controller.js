...
exports.create = async (req, res) => {
  ...
  const { name, fatherSurname, motherSurname, age, birthdate, birthplace, sex, curp, phone, hasDisability,
    disabilityType, medicalService, civilStatus, scholarship, income, address, spouseOrTutor, home, expenses, families } = req.body;

  const insertFamilies = families.map((family) => {
    return {
      ...family,
      createdBy,
      createdAt: new Date()
  ...
  const newFamilies = await Families.insertMany(insertFamilies);
...
exports.update = (req, res) => {
  ...
  Beneficiary.findOneAndUpdate(filter, beneficiary)
    ...
    .catch((err) => {
      // **** LOG **** //
      logger.log('PUT', `/beneficiary/update/${beneficiary._id}`, currentuser, err, false);
  ...
  if (userType === 'admin') {
    Beneficiary.find({ deleted: false })
      .then((beneficiaries) => {
        ...
        const paginatedData = getResultsByParameters(page, limit, sort, order, filteredData);
      ...
      .catch((err) => {
        ...
        res.status(httpStatus.INTERNAL_SERVER_ERROR).json({ error: 'internalError', message: "Error: " + err })
      })
    ...
    // **** LOG **** //
    logger.log('GET', '/beneficiaries/', username, 'Usertype not allowed', false);
    res.status(httpStatus.FORBIDDEN).json({ error: 'unauthorized', message: 'User not authorized to do this.' });
  }
}
...
function getBeneficiariesByeSearch(search, data) {
  ...
  if (search) {
    searchData = data.filter((item) => {
      const fullname = item.name + ' ' + item.fatherSurname + ' ' + item.motherSurname;
      return fullname.toLowerCase().includes(search.toLowerCase()) ||
        ...
        item.address.neighborhood.toLowerCase().includes(search.toLowerCase()) ||
        item.address.delegation.toLowerCase().includes(search.toLowerCase()) ||
        item.address.subdelegation.toLowerCase().includes(search.toLowerCase()) ||
        item.address.cp.toString().toLowerCase().includes(search.toLowerCase()) ||
